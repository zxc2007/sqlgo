...
from sqlmap.lib.core.common import arrayizeValue
from sqlmap.lib.core.common import Backend
from sqlmap.lib.core.common import filterNone
from sqlmap.lib.core.common import filterPairValues
from sqlmap.lib.core.common import flattenValue
...
from sqlmap.lib.core.common import parseSqliteTableSchema
...
from sqlmap.lib.core.common import safeSQLIdentificatorNaming
from sqlmap.lib.core.common import safeStringFormat
...
from sqlmap.lib.core.common import unsafeSQLIdentificatorNaming
...
from sqlmap.lib.core.dicts import ALTIBASE_TYPES
from sqlmap.lib.core.dicts import FIREBIRD_TYPES
from sqlmap.lib.core.dicts import INFORMIX_TYPES
...
from sqlmap.lib.core.settings import CURRENT_DB
from sqlmap.lib.core.settings import METADB_SUFFIX
from sqlmap.lib.core.settings import PLUS_ONE_DBMSES
from sqlmap.lib.core.settings import REFLECTED_VALUE_MARKER
from sqlmap.lib.core.settings import UPPER_CASE_DBMSES
from sqlmap.lib.core.settings import VERTICA_DEFAULT_SCHEMA
from sqlmap.lib.request import inject
from sqlmap.lib.utils.brute import columnExists
from sqlmap.lib.utils.brute import tableExists
from sqlmap.thirdparty import six
from src.core.parser.cmdline import db as _db
...
class Databases(object):
    ...
    def getDbs(self):
        ...
        if any(isTechniqueAvailable(_) for _ in (PAYLOAD.TECHNIQUE.UNION, PAYLOAD.TECHNIQUE.ERROR, PAYLOAD.TECHNIQUE.QUERY)):
            if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
    ...
    def getColumns(self, onlyColNames=False, colTuple=None, bruteForce=None, dumpMode=False):
        ...
        if bruteForce:
            ...
            if resumeAvailable and not colList:
                columns = {}
                ...
                if Backend.isDbms(DBMS.SQLITE):
                    if dumpMode and colList:
                        if conf.db not in kb.data.cachedColumns:
                            kb.data.cachedColumns[conf.db] = {}
                        kb.data.cachedColumns[conf.db][safeSQLIdentificatorNaming(conf.db, True)] = dict((_, None) for _ in colList)
                    else:
    ...
    def getCount(self):
        ...
        if conf.db is None and Backend.getIdentifiedDbms() not in (DBMS.SQLITE, DBMS.ACCESS, DBMS.FIREBIRD, DBMS.MCKOI, DBMS.EXTREMEDB):
            warnMsg = "missing database parameter. sqlmap is going to "
            warnMsg += "use the current database to retrieve the "
            warnMsg += "number of entries for table '%s'" % unsafeSQLIdentificatorNaming(conf.db)
            logger.warning(warnMsg)
